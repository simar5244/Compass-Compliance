...
const PAGE_SIZE = 10;
const MODULE = "content-optimization";
...
function CheckMark({ tone }: { tone: ReturnType<typeof toneOf> }) {
  ...
  if (tone === "warning") {
    return (
      <span aria-hidden className="inline-flex h-5 w-5 flex-none items-center justify-center rounded-full border border-[#9ca3af] bg-[#f3f4f6] text-[12px] font-bold text-black">
        !
      </span>
    );
  }
  return (
    <span aria-hidden className="inline-flex h-5 w-5 flex-none items-center justify-center rounded-[3px] bg-[#e5e7eb] text-[12px] font-bold text-[#374151]">
      i
    </span>
  );
}

function MonoScoreRing({ score, size = 88, stroke = 6 }: { score: number | null; size?: number; stroke?: number }) {
  ...
  return (
    ...
    >
      <svg width={size} height={size} className="-rotate-90">
        <circle cx={size / 2} cy={size / 2} r={radius} fill="#ffffff" stroke="#e5e7eb" strokeWidth={stroke} />
        <circle
          cx={size / 2}
          cy={size / 2}
          r={radius}
      ...
      <span className="absolute font-semibold text-black" style={{ fontSize: Math.max(14, size / 3.6) }}>
  ...
  return (
    <div className="relative flex-1">
      <svg
        ...
        role="img"
        aria-label="Content optimization score over time"
        onMouseLeave={() => setHovered(null)}
      >
        {[0, 50, 100].map((line) => (
          <g key={line}>
            <line x1={pad} x2={width - pad} y1={y(line)} y2={y(line)} stroke="#e5e7eb" strokeWidth="1" />
            <text x={0} y={y(line) + 3} fontSize="10" fill="#9ca3af">{line}</text>
          </g>
        ))}
      ...
      {hovered != null && (
        <div
          role="status"
          className="pointer-events-none absolute z-10 -translate-x-1/2 -translate-y-full whitespace-nowrap rounded bg-black px-2 py-1 text-[11px] font-medium text-white shadow-md"
...
export default function ContentOptimizationPage() {
      ...
      .catch((e) => {
        if (!cancelled) {
          setLoaded({
            ...
            checks: null,
            series: null,
            error: e instanceof Error ? e.message : "Failed to load content optimization",
          });
        }
      });
  ...
  if (!fresh?.checks) {
    return <CompassLoader fullPage label="Loading content optimization…" />;
  }
  ...
  return (
    <div className="light-theme px-6 py-6">
      <header className="mb-5 flex items-start gap-3">
        ...
        <div>
          <h1 className="text-[30px] font-semibold leading-9 text-black">Content optimization</h1>
          <p className="mt-1 text-[13px] text-[#6b7280]">Marketing · content editors</p>
        </div>
      </header>

      <section className="mb-5 rounded-[6px] border border-[#e5e7eb] bg-white p-6">
        <h2 className="text-[18px] font-semibold text-black">Optimize your content</h2>
        <p className="mt-1.5 max-w-[70ch] text-[14px] leading-[1.6] text-[#4b5563]">
          Improve the marketing of content on this website. This ignores technical issues and focuses
          on areas content editors can improve.
        </p>
      </section>
      ...
      <div className="mb-5 grid gap-5 lg:grid-cols-[minmax(0,260px)_minmax(0,1fr)]">
        <section className="rounded-[6px] border border-[#e5e7eb] bg-white p-5">
          <h3 className="text-[15px] font-semibold text-black">Content optimization</h3>
          <div className="mt-3 flex flex-col items-center">
            <MonoScoreRing score={latest} />
            {delta != null && delta !== 0 && (
              <p className="mt-2 text-[11px] font-medium text-[#374151]">
        ...
        <section className="flex min-h-[160px] flex-col rounded-[6px] border border-[#e5e7eb] bg-white p-4">
          <h3 className="mb-2 text-[13px] font-medium text-black">Score over time</h3>
      ...
      <div className="rounded-[6px] border border-[#e5e7eb] bg-white">
        ...
        {tab === "Checks" ? (
          <>
              ...
              <div className="ml-auto flex items-center gap-2">
                {searchOpen && (
                  <input
                    ...
                    className="h-9 w-[200px] rounded-[4px] border border-[#d1d5db] bg-white px-3 text-sm text-black outline-none focus:border-black"
                  />
                )}
                <button type="button" aria-label="Columns" className="flex h-9 items-center gap-1.5 rounded-[4px] border border-[#d1d5db] px-3 text-[13px] font-medium text-[#374151]">
                  <SlidersHorizontal aria-hidden className="h-4 w-4" /> Columns
                </button>
                <button
                  type="button"
                  aria-label="Search"
                  onClick={() => setSearchOpen((previousOpen) => !previousOpen)}
                  className="grid h-9 w-9 place-items-center rounded-[4px] border border-[#d1d5db] text-[#374151]"
                >
                  <Search aria-hidden className="h-4 w-4" />
                </button>
                <button type="button" aria-label="More options" className="grid h-9 w-9 place-items-center rounded-[4px] border border-[#d1d5db] text-[#374151]">
            ...
            <div className="h-[calc(100vh-420px)] min-h-[220px] overflow-auto">
              <table className="w-full border-t border-[#e5e7eb] text-left">
                <thead className="sticky top-0 z-10">
                  <tr className="bg-[#fafafa] text-[13px] text-[#6b7280]">
                ...
                <tbody>
                  {slice.map((row) => (
                    <tr key={row.check_id} className="border-t border-[#f3f4f6] hover:bg-[#fafafa]">
                        ...
                        <span className="flex items-start gap-2.5">
                          ...
                          <span>
                            <button
                              ...
                              className="text-left text-[14px] font-medium text-black underline-offset-2 hover:underline"
                            >
            ...
            {pageCount > 1 && (
              <nav aria-label="Pagination" className="flex flex-wrap items-center justify-center gap-1 border-t border-[#e5e7eb] py-4">
                {Array.from({ length: pageCount }).map((_, index) => (
                  <button
                    ...
                    className={`h-8 min-w-8 rounded-[4px] px-2 text-[13px] font-medium ${
                      index === current ? "bg-black text-white" : "text-[#374151] hover:bg-[#f3f4f6]"
                    }`}
                  >
                    {index + 1}
                  </button>
                ...
                {withIssues.map((row) => (
                  <li key={row.check_id} className="flex items-center gap-3 py-3">
                    ...
                    <button
                      type="button"
                      onClick={() => router.push(`/sites/${params.id}/checks/${row.check_id}`)}
                      className="flex-1 text-left text-[14px] font-medium text-black underline-offset-2 hover:underline"
                    >
                      {row.display_name ?? row.check_id}
                    </button>
